...
async function startCallTimer (io, logId, callerId, receiverId, maualStopFlag) {
    ...
    callTimers[logId] = setInterval(async () => {
      ...
      {        
        await User.findByIdAndUpdate(callerId, {
                $inc: { usedCallMinutes: caller.videoCallMinutes }, //CallerusedCallSeconds
                $set: {ManualStopFlag: true, videoCallMinutes: 0}             
              });
        ...
        console.log(
          "⛔ Minutes exhausted. Ending call.",
          CallerusedCallSeconds,
        );
